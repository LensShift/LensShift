...
RSpec.describe "resource_items/new", type: :view do
  before(:each) do
    assign(:resource_item, ResourceItem.new(
      :title => "MyString",
      :author => "MyString",
      :source => "MyString",
      :source_url => "MyString",
  ...
  it "renders new resource_item form" do
    render

    assert_select "form[action=?][method=?]", resource_items_path, "post" do
